...
export class OrderService {
  ...
  //
  // async createOrderv1(dto: CreateOrderDtov1): Promise<Order> {
  ...
  async createOrderv2(dto: CreateOrderDtov2): Promise<Order> {
    ...
    const orderItems = await Promise.all(
      cart.map(async (cartItem) => {
        ...
        orderItem.product = await Product.findOneBy({
        ...
        if (orderItem.product.salePrice) {
          discount +=
            (orderItem.product.originalPrice - orderItem.product.salePrice) *
            cartItem.quantity;
        }
        subTotal += orderItem.product.originalPrice * cartItem.quantity;
    ...
    // if (dto.paymentMethod === 'BANKING') {
    //   order.status = OrderStatus.PENDING;
    //   const payment = await this.PaymentService.createPayment(res);
    //   res.paymentRef = payment.paymentLinkId;
    //   res.save();
    // }
  }

  async paymentReference(order: Order, ref: string): Promise<Order> {
